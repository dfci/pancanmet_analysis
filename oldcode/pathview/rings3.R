...
rings3 = function( ringdata,pathway,path2save ){
  ...
  if (length(grep('jpg',path2save))>0){w = 2;h = 2}else{w=7;h=7}
  ...
  print( ggplot(plotdata1, aes(fill=V2, ymax=width, ymin=c(0, head(width, n=-1)), xmax=5, xmin=3)) +
           geom_rect(colour="grey30") + #optionally change geom_rect size = .1
           ...
           scale_fill_gradient2(low = '#0000FF', mid = '#FFFFFF', high = '#FF0000', 
               guide = guide_legend(title = "Log2 FC"),midpoint = 0,na.value = "#6B6B6B",limits = c(minval,maxval))  +
           geom_rect(data=plotdata2, colour="grey30", xmax=5, xmin=3, aes(ymax=width, ymin=c(0.5, head(width, n=-1)))) +
           geom_rect(data = plotdata3, colour="grey10", xmax=2.5, xmin=0, aes(ymax=0.5, ymin= 0)) + 
           geom_rect(data = plotdata4, colour="grey10", xmax=2.5, xmin=0, aes(ymax=1, ymin= 0.5)) + 
           #ggtitle(pathway) + 
           geom_vline(xintercept=5) + 
           geom_vline(xintercept=3) +
           geom_text(data = plotdata2,aes(label = Name, x = 5, y = width - 0.25/length(width)),size = 3) + 
           geom_text(data = plotdata1,aes(label = Name, x = 4.5, y = width - 0.25/length(width)),size = 3) + 
