# A script to generate a summary of the merged metabolomics data
...
cancers <- as.factor(sapply( strsplit(allSamples, "\\."), FUN=function(x) { return(names2plot[x[1]]); }));
cancerTypes <- levels(cancers);

# and this is all unnormalized data (t and n)
hugeData <- as.matrix(read.csv("../data/merged_metabolomics/merged_metabolomics.csv", header=T, row.names=1));
...
# hugeCancers <- as.factor(sapply( strsplit(hugeSamples, "\\."), FUN=function(x) { return(x[1]); }));
hugeCancers <- as.factor(sapply( strsplit(hugeSamples, "\\."), FUN=function(x) { return(names2plot[x[1]]); }));
...
tissue_v_cancer <- table(hugeCancers,sampleTypes)
...
allSampleCounts["Tumor", names(sampleCounts)] <- tissue_v_cancer[names(sampleCounts),'Tumor'];
allSampleCounts["Normal", names(sampleCounts)] <- tissue_v_cancer[names(sampleCounts),'Normal'];
