...
gvdata = readWorksheetFromFile('checkdata/GoveiaData.xlsx',sheet = 'Sheet1',rownames = 1)
...
ggplot(gvdata,aes(Reznik_DAScore,Goveia_DAScore, label = gvnames)) + 
   geom_point(aes(size = -LogP)) + theme_bw(base_size = 18) + 
   geom_hline(yintercept = 0) + geom_vline(xintercept = 0) + 
   scale_color_gradient2() + geom_text_repel(color = 'black') + 
